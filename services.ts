...
export async function getSongInfo(id: string): Promise<Song> {
  try {
    ...
    console.error(
      '获取歌曲信息失败 Failed to get song info:',
...
async function getSongUrl(id: string, level: string): Promise<string | null> {
  try {
    ...
    if (!songData?.url) {
      console.log(`未获到 ${level} 音质的 URL`);
      return null;
    }

    console.log(
      `获取到音质 Quality: ${level}, 比特率 Bitrate: ${Math.floor(songData.br / 1000)}kbps, 格式 Format: ${songData.type}, URL: ${songData.url}`,
    );
    return songData.url;
  } catch (error) {
    console.error(
      `尝试获取 ${level} 音质失败 Failed to get ${level} quality:`,
      error instanceof Error ? error.message : 'Unknown error',
    );
    return null;
  }
...
export async function getLyrics(id: string): Promise<string | null> {
  try {
    ...
    if (result?.code !== 200) {
      console.error('获取歌词失败 Failed to get lyrics:', result?.message || 'Unknown error');
      return null;
    }
